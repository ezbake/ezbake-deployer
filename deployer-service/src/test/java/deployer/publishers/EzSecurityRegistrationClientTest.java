...
public class EzSecurityRegistrationClientTest {
    ...
    public void testCertificates() throws DeploymentException {
        List<CertDataEntry> certs = client.get(TestUtils.getTestEzSecurityToken(), TestUtils.APP_NAME, TestUtils.SECURITY_ID);

        assertEquals(7, certs.size());

        Hashtable<String, CertDataEntry> temp = new Hashtable<String, CertDataEntry>();
        Hashtable<String, Integer> sizes = new Hashtable<String, Integer>();
        for (CertDataEntry c : certs) {
    ...
    public void assertEntry(Hashtable<String, CertDataEntry> temp, Hashtable<String, Integer> expected, String key) {
        CertDataEntry entry = temp.get(key);
