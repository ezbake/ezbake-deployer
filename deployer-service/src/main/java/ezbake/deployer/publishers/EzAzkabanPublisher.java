...
public class EzAzkabanPublisher implements EzPublisher{
    ...
    public void publish(DeploymentArtifact artifact, EzSecurityToken callerToken) throws DeploymentException {
        ...
        try {
            ...
            if(jobInfo.isSetStartDate()){ scheduler.setScheduleDate(jobInfo.getStartDate()); }
            if(jobInfo.isSetStartTime()){ scheduler.setScheduleTime(jobInfo.getStartTime()); }
            if(jobInfo.isSetRepeat()){    scheduler.setPeriod(jobInfo.getRepeat()); }
